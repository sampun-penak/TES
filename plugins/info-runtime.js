...
let name = await conn.getName(who)
    ...
    let thm = ['1st-World-Canadian-Problems',
'2nd-Term-Obama',
'10-Guy',
'1950s-Middle-Finger',
...
let res = `https://apimeme.com/meme?meme=${thm.getRandom()}&top=Runtime&bottom=${muptime}`

    try {
				const stek = new Sticker(encodeURI(res), { pack: packname, author: author, type: StickerTypes.FULL })
				const buffer = await stek.toBuffer()
				conn.sendFile(m.chat, buffer, 'sticker.webp', '', fakes, adReply, { asSticker: true })
			} catch {
				let stiker = await sticker(null, encodeURI(res), global.packname, global.author)
