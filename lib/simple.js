...
END:VCARD`.trim()
    ...
    */
    ...
    let file = await conn.resize(buff, 450, 300)
    let a = [1,2]
    ...
    */
	...
	let file = await conn.resize(image, 450, 300)
...
/**
    ...
    */
    sendVote: {
            async value(jid, name = '', optiVote, options) {
                if (!Array.isArray(optiVote[0]) && typeof optiVote[0] === 'string') optiVote = [optiVote]
                if (!options) options = {}
                const pollCreationMessage = {
        name: name,
        options: optiVote.map(btn => ({
                            optionName: !nullish(btn[0]) && btn[0] || !nullish(btn[1]) && btn[1] || ''
                    })),
                    selectableOptionsCount: num ? num : 1
        }
                return conn.relayMessage(jid, { pollCreationMessage }, { ...options });
                }
        },
    /**
    * Send List
    ...
    */
    ...
    async value(jid, buffer, content, footer, button1, row1, quoted, options = {}) {
        let file = await conn.resize(buffer, 450, 300)
        let buttons = [
